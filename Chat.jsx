...
const Chat = () => {
  ...
  const [speachOn, setSpeechon] = useState(false);
  ...
  useEffect(() => {
    if (speachOn) {
      const lastResponseBox = document.querySelector('.chat_box .box:last-child');
      if (lastResponseBox) {
        speak(lastResponseBox.innerText);
      }
      setSpeechon(false);
    }
  }, [speachOn]);
  ...
  return (
    <div className='chat_cont'>
      <nav className='nav'>
        ...
        <button onClick={() => { setSpeechon(true) }} style={{
          width:"50px",
          height:"50px",
          borderRadius:'50%',
          marginRight:"20px",
